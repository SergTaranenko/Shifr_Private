...
class GigaChatAPI:
    ...
    async def generate_image(self, prompt: str):
        ...
        if not token:
            logger.warning("No GigaChat token — image skipped")
            return None
        ssl_context = ssl.create_default_context()
        ssl_context.check_hostname = False
        ssl_context.verify_mode = ssl.CERT_NONE
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(
                    ...
                    if resp.status != 200:
                        ...
                        logger.error(f"GigaChat image HTTP {resp.status}: {body[:200]}")
                        return None
                    data = await resp.json()
                    ...
                    if "<img src=\"" in content:
                        ...
                        logger.info(f"GigaChat image file_id: {file_id}")
                        async with session.get(
                            ...
                            if img_resp.status == 200:
                                ...
                                logger.error(f"Image download HTTP {img_resp.status}")
                    else:
                        logger.warning(f"No <img> in response. Content: {content[:200]}")
        except Exception as e:
